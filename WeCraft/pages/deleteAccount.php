<?php
  ...
  if($kindOfTheAccountInUse == "Guest"){
    ...
    //Check that the artisan has not claimed a non complete project
    $checkProjectsNonCompletedAndClaimedByThisArtisanIsOk = true;
    if($kindOfTheAccountInUse == "Artisan"){
      $numProjectsNonCompletedAndClaimedByThisArtisan = numberProjectsNonCompletedAndClaimedByThisArtisan($_SESSION["userId"]);
      if($numProjectsNonCompletedAndClaimedByThisArtisan > 0){
        $checkProjectsNonCompletedAndClaimedByThisArtisanIsOk = false;
      }
    }
    if($checkProjectsNonCompletedAndClaimedByThisArtisanIsOk == false){
      addParagraph(translate("You cant delete the account because you have some claimed projects for personalized items not completed"));
    } else {
